...
| `TransferProgressWindow` | `src/ui/TransferProgressWindow.hpp` | Non-modal `Qt::Tool` window listing all transfer jobs with per-row status icon, progress bar, cancel/retry buttons, auto-scroll checkbox, and a "Stay on top" checkbox. Created with `parent = nullptr` so it is not minimized together with `MainWindow`. Auto-scroll is driven by a single permanent `QScrollBar::rangeChanged` connection (not a per-item one-shot); `scrollToActive()` is called from that handler, from `checkStateChanged` when the checkbox is re-enabled, and from `on_clearFinished_clicked`. `scrollToActive()` prefers the last Running row (live progress) and falls back to the last Queued row when nothing is running. Manual scroll (wheel or scrollbar drag) unchecks auto-scroll. The "Stay on top" checkbox sets/clears `Qt::WindowStaysOnTopHint` and persists the choice via `AppConfig` (`ui/transfers_stay_on_top`); on Wayland it is disabled with a tooltip because `Qt::WindowStaysOnTopHint` has no cross-process effect under the XDG-shell protocol. |
