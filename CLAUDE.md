...
| `TransferProgressWindow` | `src/ui/TransferProgressWindow.hpp` | Non-modal `Qt::Tool` window listing all transfer jobs with per-row status icon, progress bar, cancel/retry buttons, and an auto-scroll checkbox. Auto-scroll is driven by a single permanent `QScrollBar::rangeChanged` connection (not a per-item one-shot); `scrollToActive()` is called from that handler, from `checkStateChanged` when the checkbox is re-enabled, and from `on_clearFinished_clicked`. `scrollToActive()` prefers the last Running row (live progress) and falls back to the last Queued row when nothing is running. Manual scroll (wheel or scrollbar drag) unchecks auto-scroll. |
